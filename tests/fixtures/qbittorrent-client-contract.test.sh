...
probe_project="${project_root}/tests/Jellyfin.Plugin.QControl.QBittorrentContractProbe/Jellyfin.Plugin.QControl.QBittorrentContractProbe.csproj"
completed=0
    ...
    if [[ "${completed}" -ne 1 ]]; then
        ...
        docker compose \
            ...
            | sed --regexp-extended \
                ...
                >"${project_root}/.testenv/last-qbittorrent-client-failure.log" || true
    fi
...
completed=1
printf 'Verified every QControl qBittorrent endpoint through the typed client.\n'
